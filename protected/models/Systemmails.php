...
class Systemmails extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, description, subject, message, attributes', 'required'),
			array('name', 'length', 'max'=>128),
			array('description', 'length', 'max'=>256),
			array('subject', 'length', 'max'=>64),
			array('attributes', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, description, subject, message, attributes', 'safe', 'on'=>'search'),
		);
	}
