...
class AssignmentController extends RController
{
	...
	*/
	...
	{
	$usermember_ship=Yii::app()->db->createCommand("select * from user_membership")->queryAll();
		
		 $date2 = strtotime(date("Y-m-d H:i:s"));
		 foreach($usermember_ship as $val)
		 {
			 if(strtotime($val['end_date'])<$date2)
				{
					$command_table=Yii::app()->db->createCommand("UPDATE `authassignment` SET `itemname`='SM' where
					userid='".$val['user_id']."'");
            		$command_table->execute();
				}
				else
				{
					$command_table=Yii::app()->db->createCommand("UPDATE `authassignment` SET `itemname`='PM' where
					userid='".$val['user_id']."'");
            		$command_table->execute();
	...
	public function actionUser()
	{
		...
		if( $assignSelectOptions!==array() )
		{
		    ...
		    if( isset($_POST['AssignmentForm'])===true )
			{
				...
				{
					// Update and redirect
					if($this->_authorizer->authManager->assign($formModel->itemname, $model->getId()))
					{
					// Update UserMembership
					if($formModel->itemname=='PM')
					{
						$dateend_date = date("Y-m-d H:i:s",strtotime("+1 month",strtotime(date("Y-m-d"))));
						$command_table=Yii::app()->db->createCommand("UPDATE `user_membership` SET membe_fees = '0.00',duration = '1 month',token  = '',start_date  ='".date("Y-m-d H:i:s")."',status ='success',end_date  ='".$dateend_date."' where user_id='".$model->getId()."'");
            		$command_table->execute();
					
					}elseif($formModel->itemname=='SM'){
						$dateend_date1 = date("Y-m-d H:i:s",strtotime("-1 day",strtotime(date("Y-m-d"))));
						$command_table=Yii::app()->db->createCommand("UPDATE `user_membership` SET membe_fees = '0.00',duration = '0',token  = '',start_date  ='".date("Y-m-d H:i:s")."',status ='Pending',end_date  ='".$dateend_date1."' where user_id='".$model->getId()."'");
						$command_table->execute();
						}
					}
					//////////////////////////////////////////////
