...
$(function() {
	...
	//alert(actionForUpload);
	function nocache() 
	{
		var d = new Date();
		return "<?php echo $url_separator; ?>ajax_upload=2&<?php echo $csrf; ?>&field=<?php echo $field->varname ?>&t=" 
		+ d.getTime();
	}
	...
	function generateThumb(c) {
		updatePreview(c);
		$("#thumb_loading").show();
		$.get("<?php echo Yii::app()->request->requestUri . '?'.$url_separator; ?>ajax_upload=2&field=<?php echo $field->varname ?>&thumb=1&x="+c.x+"&y="+c.y+"&w="+c.w+"&h="+c.h+"",function() {
			$("#thumb_loading").hide();
		});
	}
	...
	var uploader = new qq.FileUploader({
		...
		console.log(id+'::'+fileName+'::'+result.fullpath);
