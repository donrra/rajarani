...
class Message extends CActiveRecord
{
	...
	public $m_id,$m_body,$m_sender_id,$m_receiver_id,$m_subject,$m_is_read,$m_parent_id,$m_is_spam;
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, sender_id, receiver_id, subject, body, is_read, deleted_by,parent_id, created_at,type', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function getAdapterForMessageHistory($messageId,$message)
	{
			$messages=Yii::app()->db->createCommand("select * from  messages where message='".$message."' order by id DESC")->queryAll();
			return $messages;
	}


public static function getAdapterForSpam($userId) {

		$c = new CDbCriteria();
		$c->addCondition('t.receiver_id = :receiverId');
		$c->addCondition('t.is_spam = 1');
		//$c->addCondition('t.parent_id = 0 ');
	...
	public static function getAdapterForInbox($userId) {
				...
				$c->addCondition('id IN(SELECT MAX(id) AS id FROM messages where receiver_id = :receiverId GROUP BY message)');
				$c->addCondition('t.id = id');
				$c->addCondition('t.receiver_id = :receiverId');
				$c->addCondition('t.is_spam = 0');
				$c->addCondition('t.deleted_by <> :deleted_by_receiver OR t.deleted_by IS NULL');
				$c->order='id desc';
				$c->params = array(
					'receiverId' => $userId,
	...
	public function markAsSpam() {
		if (!$this->is_spam) {
			$this->is_spam = 1;
			$this->save();
			// need to send a message to the admin regarding message and who make it as a spam. 
			// pending ....
