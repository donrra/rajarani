					...
					<div class="left_block equal">
                        ...
                        <section class="content_block2">
                            ...
                            <ul class="msg_lists">
								...
								//echo '<pre>';print_r($las_blockEntries);
							 ...
							 ?><?php if($message->parent_id!=0 && $message->reciever_viewed==Yii::app()->user->getId())
							  {?>
                                <li class="<?php echo 'read' ?>" id="msglist_<?php echo $message->id; ?>">
                              <?php }elseif($message->parent_id!=0 && $message->receiver_id==Yii::app()->user->getId() && $message->is_read=='')
							  {?>
                              <li class="<?php echo 'read' ?>" id="msglist_<?php echo $message->id; ?>">
							  <?php 
							  }elseif($message->parent_id!=0 && $message->reciever_viewed==0)
							  { ?>
...
$(document).ready(function() {
	...
	$("div .msg_block").click(function() {
		...
		$.ajax({
		type : 'GET', 
		url: "<?php echo Yii::app()->request->baseUrl;?>/message/view/messageviewed?message_id="+id,
		data: {},
		success: function(){
			location.href="<?php echo Yii::app()->request->baseUrl;?>/message/view?message_id="+id;
			}
		})
		return false;
