...
{ 
	...
	public function actionSpam($id) {
		...
		$message->markAsSpam();
		Yii::app()->user->setFlash('messageModule', MessageModule::t('Message has been marked as spam'));
	}
	public function actionViewspam()
	{
		...
		$messagesAdapter = Message::getAdapterForSpam(Yii::app()->user->getId());
		$pager = new CPagination($messagesAdapter->totalItemCount);
		$pager->pageSize = 10;
		$messagesAdapter->setPagination($pager);

		$this->render(Yii::app()->getModule('message')->viewPath . '/viewspam', array(
