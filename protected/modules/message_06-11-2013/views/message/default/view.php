					...
					<aside class="msg_profile">
								...
								if($viewedMessage->getSenderName()=='admin') 
								{
									 echo ($viewedMessage->getSenderName()=='admin') ? 'The Rajarani Team':$viewedMessage->getSenderName() ;  
										...
										echo CHtml::link( ($viewedMessage->getSenderName()=='admin') ? 'The Rajarani Team':$viewedMessage->getSenderName(),array('/user/profile/'.$viewedMessage->getSenderName())); 
									}?></h4>
                                <?php else: ?>
                                ...
                                <?php endif; ?>
									...
									<?php
										...
										{
											echo strrev(stristr(strrev(substr(CHtml::encode($viewedMessage->subject),
											0,30)),' '))." ...";
									...
									<p class="date">
                                    <?php $tmp=date(Yii::app()->getModule('message')->dateFormat, strtotime($viewedMessage->created_at));					$date = new DateTime($tmp);
								...
								<div class="right top_space" id="dropdown">
									<div class="dropdown s_dropdown"  id="vmaction">
										...
										<ul>
											...
											{
                                            if($user_Senderon){?>
                                            <li id="chat"><span onclick="javascript:chatWith('<?php echo $viewedMessage->getSenderName(); ?>')">Chat</span></li><?php }}elseif($viewedMessage->receiver_id!=Yii::app()->user->getId())
											{ if($user_Receiveron){?><li id="chat"><span onclick="javascript:chatWith('<?php echo $viewedMessage->getReceiverName(); ?>')">Chat</span></li><?php }}?>
                        ...
                        <section class="content_block2">
							...
							echo nl2br(CHtml::decode($viewedMessage->body));
							  ?></p>
								...
								<?php foreach ($messagehistory->data as $index => $histmessage): ?>
                               <div class="msg_yellow">
									<h5><?php echo $histmessage->getSenderName(); ?> 
                                    <span><?php echo MessageModule::t('Replied'); ?> <?php 
									echo compare_dates(strtotime($histmessage->created_at));?></span>
                                    </h5>
								...
								<?php
								if($viewedMessage->type!=1 && Yii::app()->user->checkAccess('Message.Compose.Compose'))
                                {
