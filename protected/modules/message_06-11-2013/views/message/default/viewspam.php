					...
					<div class="left_block equal">
                        ...
                        <section class="content_block2">
                            ...
                            <ul class="msg_lists">
							 ...
							 $ls_sqlBlocks = "SELECT * FROM `messages` WHERE `is_spam`=1 AND `parent_id`=$message->id AND `receiver_id`=$message->receiver_id order by `created_at` DESC"; 							   
							 ...
							 ?>
                             <li class="<?php echo $chMessage->is_read ? 'read' : 'unread' ?>" id="msglist_<?php echo $chMessage->id; ?>">
 <?php
 $sendermodel = User::model()->findByAttributes(array('id'=>$chMessage->sender_id));
if($sendermodel->profile->getAttribute('avatar')!=NULL)
{
	$circularthumbimg=Yii::app()->request->baseUrl.'/'.$sendermodel->profile->getAttribute('avatar');
}else{
...
?>   				<div class="pro_img circular" style="background: url(<?php echo $circularthumbimg; ?>) no-repeat;">
					<img src="<?php echo $circularthumbimg; ?>" />
                                    ...
                                    <?php echo $form->hiddenField($message,"[$index]id"); ?>
                                       ...
                                       <!-- <p class="msg"><a href="<?php echo $this->createUrl('_view/', array('message_id' => $message->id)) ?>"><?php echo $message->subject ?></a></p>-->
										<p class="date">
									<?php
...
if($sendermodel->profile->avatar !='')
{
	$circularthumbimg=Yii::app()->request->baseUrl.'/'.$sendermodel->profile->avatar ;
		...
		$circularthumbimg=Yii::app()->request->baseUrl.'/user_avatar/thumb/default.png';
}
/**/?>   				<div class="pro_img circular" style="background: url(<?php echo $circularthumbimg; ?>) no-repeat;">
					<img src="<?php echo $circularthumbimg; ?>" />
                                    </div>
...
$(document).ready(function() {
	...
	$("div .msg_block").click(function() {
			location.href="<?php echo Yii::app()->request->baseUrl;?>/message/spamview?message_id="+$(this).attr('id');
		}); 
