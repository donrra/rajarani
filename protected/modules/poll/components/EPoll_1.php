...
Yii::import('zii.widgets.RPortlet');

class EPoll extends RPortlet
{
  /**
  ...
  public function init()
  {
    ...
    if ($this->_poll) {
     $this->title = UserModule::t($this->_poll->title);//$this->_poll->title;
   }
    ...
    if ($model) {
	  ...
	  if (Yii::app()->getModule('poll')->forceVote && $model->userCanVote()) {
        ...
        foreach ($model->choices as $choice) {
          $choices[$choice->id] = CHtml::encode(UserModule::t($choice->label));
        }
