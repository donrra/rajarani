	...
	public function actionSearch()
	{
		 ...
		 if($f_age!='' && $t_age!='')
		{
		 ...
		 $user_profile=array();
		  ...
		  if($whereclause!='')
		 {
			 $ls_searchprogile="SELECT t.user_id,s.online FROM profiles `t` LEFT JOIN online_users `s` ON t.user_id=s.user_id LEFT JOIN users `u` ON t.user_id=u.id WHERE $whereclause and t.user_id!='".Yii::app()->user->getId()."' and u.superuser=0 and status NOT IN (0,2)";
